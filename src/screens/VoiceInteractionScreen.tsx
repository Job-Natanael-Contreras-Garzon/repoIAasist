...
const VoiceInteractionScreen: React.FC<Props> = ({ route }) => {
  ...
  const onVoiceResults = React.useCallback((results: string[]) => {
    if (results && results.length > 0) {
      ...
      addInteraction('user', recognizedText);
      // Process the command immediately without dependency
      setTimeout(() => {
        handleVoiceCommand(recognizedText.toLowerCase());
      }, 0);
    }
  ...
  const onVoiceError = React.useCallback((error: any) => {
    ...
    setTimeout(() => {
      // Use ref or direct call to avoid dependency
      startListening();
    }, 2000);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [addInteraction]);
  ...
  }, [emergencyStarted, currentStep, instructions, offerEmergencyCall, startEmergencyGuidance, startNextInstruction, addInteraction, startListening]);
  ...
  const handleConnectivityChange = React.useCallback((connected: boolean) => {
    setIsConnected(connected);
    setOfflineMode(!connected);
    
    if (!connected) {
      addInteraction('system', 'Modo offline activado. Funciones básicas disponibles.');
    } else {
      addInteraction('system', 'Conexión restaurada. Todas las funciones disponibles.');
    }
  }, [addInteraction]);

  // Manual emergency call
  const handleManualEmergencyCall = React.useCallback(() => {
    Alert.alert(
      'Llamar a Emergencias',
  ...
  useEffect(() => {
    const loadInstructions = async () => {
      if (categoryId) {
        if (isConnected) {
          // Use local instructions when connected
          setInstructions(firstAidInstructions[categoryId] || []);
        } else {
          // Use offline service when disconnected
          const offlineInstructions = await offlineService.getOfflineInstructions(categoryId);
          setInstructions(offlineInstructions);
        }
        startEmergencyGuidance();
      } else {
  ...
  }, [categoryId, isConnected, startEmergencyGuidance, startGeneralEmergencyAssistance]);
